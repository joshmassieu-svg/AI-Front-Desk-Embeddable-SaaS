...
export async function POST(req: NextRequest) {
  try {
    ...
    // PAIN POINT (fixed): this used to trust whatever conversationId the
    // client sent (from localStorage) over the conversation actually
    // resolved above for this (websiteId, visitorId) pair. If the stored
    // id was stale — e.g. the conversation was deleted from the dashboard,
    // or the visitor switched devices/cleared storage on this browser only
    // — messages got written to a conversation addMessageAsync couldn't
    // find (silently dropped, no error to the visitor) while the chat
    // history sent to Gemini came from a *different* conversation than the
    // one being written to. The freshly-resolved conv.id is always the
    // source of truth; the client-supplied id is only used to restore
    // history/continuity when it actually matches.
    if (conversationId && conversationId !== conv.id) {
      console.warn(`[chat/stream] PAIN POINT: client-supplied conversationId=${conversationId} does not match resolved conversationId=${conv.id} for visitorId=${visitorId} — using resolved id, client's stale id is discarded`);
    }
