...
export default function CustomizerPage() {
  ...
  const presetBundles = [
    ...
    },
    {
      ...
      welcomeMessage: 'Hi there! Have a question? Ask me anything.',
    },
    {
      id: 'retail',
      name: '🛍️ E-Commerce',
      ...
      launcherStyle: 'tab',
      borderRadius: 12,
      botName: 'Sales Specialist',
      welcomeMessage: 'Welcome! Ask about order tracking, discounts, or returns.',
    },
    {
      id: 'executive',
      name: '💼 Corporate',
      ...
      borderRadius: 8,
      botName: 'Client Concierge',
      welcomeMessage: 'Hello. How may our enterprise team assist you today?',
    },
    {
      id: 'cyberpunk',
      name: '⚡ Cyberpunk',
      ...
      launcherStyle: 'bar',
      borderRadius: 20,
      launcherTheme: 'cosmic',
      enableParticleTrail: true,
      botName: 'AI Cyber',
      welcomeMessage: '⚡ System Online. Accessing Flowdexx RAG database...',
    },
  ];
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto font-sans pb-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* ================= COLUMN 1: LEFT NAV & PRESETS (3 COLS) ================= */}
        <div className="lg:col-span-3 space-y-5">
          ...
          <div className="glass-panel p-4 rounded-2xl border border-amber-200/90 bg-[#FFFFE0]/80 shadow-sm space-y-3">
            <div className="flex items-center gap-1.5 text-amber-900 text-xs font-bold">
              <Zap className="w-3.5 h-3.5 text-amber-700" /> Instant Theme Presets
            </div>
            <p className="text-[11px] text-slate-600 leading-snug">
              Click any style bundle below to apply colors, greetings, and animations in 1-click:
            </p>
            <div className="space-y-1.5">
              {presetBundles.map((pb) => (
                <button
                  key={pb.id}
                  onClick={() => applyPreset(pb)}
                  className="w-full px-3 py-2 rounded-xl bg-white hover:bg-amber-100/70 border border-amber-200/90 text-slate-800 text-xs font-semibold transition text-left flex items-center justify-between shadow-xs cursor-pointer"
                >
                  <span>{pb.name}</span>
                  <span className="w-2.5 h-2.5 rounded-full border border-amber-300" style={{ backgroundColor: pb.primaryColor }} />
                </button>
              ))}
            </div>
          </div>
