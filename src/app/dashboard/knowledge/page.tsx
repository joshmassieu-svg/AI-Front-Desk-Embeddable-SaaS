  ...
  const handleCrawl = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/v1/knowledge/crawl', {
        ...
        body: JSON.stringify({ websiteId: currentSiteId, url: crawlUrl, mode: crawlMode }),
      });
      const data = await res.json();
      if (!res.ok) {
        setCrawlSummary({ error: data.error || 'Crawl failed' });
      } else if (crawlMode === 'site') {
        setCrawlSummary({ pagesIndexed: data.pagesIndexed, pagesFailed: data.pagesFailed });
      } else {
        setCrawlSummary({ pagesIndexed: 1, pagesFailed: 0 });
      }
      setCrawlUrl('');
      fetchItems();
    } catch (err) {
      console.error(err);
      setCrawlSummary({ error: 'Crawl request failed' });
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="glass-panel p-6 rounded-2xl border border-slate-800 max-w-3xl space-y-4">
          ...
          <div className="flex gap-2 text-xs">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg border ${crawlMode === 'single' ? 'bg-brand-600 border-brand-500 text-white' : 'border-slate-700 text-slate-400'}`}
            >
              Single page
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-lg border ${crawlMode === 'site' ? 'bg-brand-600 border-brand-500 text-white' : 'border-slate-700 text-slate-400'}`}
            >
              Crawl entire site
            </button>
          </div>
          ...
          {crawlSummary && (
            <div className={`text-xs rounded-lg p-3 border ${crawlSummary.error ? 'border-red-800 bg-red-950/40 text-red-300' : 'border-emerald-800 bg-emerald-950/30 text-emerald-300'}`}>
              {crawlSummary.error
