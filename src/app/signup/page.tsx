  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (info && !info.isNewUser) {
        // User already has an existing account -> prevent login via signup page
        await logout();
    ...
    <>
      <style jsx global>{`
        ...
        @keyframes meshFloatThree {
          0%, 100% { transform: translate3d(0, 0, 0) scale(1); }
          50% { transform: translate3d(35px, 50px, 0) scale(0.95); }
        }
        @keyframes meshFloatFour {
          0%, 100% { transform: translate3d(0, 0, 0) scale(1); }
          50% { transform: translate3d(-40px, -40px, 0) scale(1.05); }
        }
        .mesh-one { animation: meshFloatOne 11s ease-in-out infinite; }
        .mesh-two { animation: meshFloatTwo 13s ease-in-out infinite; }
        .mesh-three { animation: meshFloatThree 15s ease-in-out infinite; }
        .mesh-four { animation: meshFloatFour 12s ease-in-out infinite; }
      `}</style>
        ...
        <div className="min-h-[calc(100vh-1.5rem)] sm:min-h-[calc(100vh-2rem)] lg:min-h-[calc(100vh-2.5rem)] w-full overflow-hidden rounded-[28px] border border-pink-100 bg-white shadow-[0_20px_80px_rgba(244,114,182,0.08)] flex flex-col lg:flex-row">
          ...
          <section className="flex min-h-[calc(100vh-1.5rem)] flex-1 items-center justify-center bg-white px-5 py-10 sm:px-10 lg:min-h-0 lg:px-12 xl:px-20">
            <div className="w-full max-w-[440px]">
              ...
              {/* Login / Signup tabs */}
              <div className="mt-8 grid grid-cols-2 rounded-xl bg-amber-50/60 p-1 border border-amber-100">
                <Link
                  href="/login"
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <label className="flex cursor-pointer items-start gap-3 pt-1">
                  <input
                    ...
                    className="mt-0.5 h-4 w-4 shrink-0 cursor-pointer appearance-none rounded border border-slate-300 bg-white transition checked:border-pink-500 checked:bg-pink-500 focus:ring-2 focus:ring-pink-500/20"
                  />
