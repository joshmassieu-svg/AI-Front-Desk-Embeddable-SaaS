...
// PAIN POINT: previously there was no durable visitor identity at all —
// every visitor fell back to a shared 'vis_anon' id server-side, which
// caused different visitors' conversations to collide (see chat/stream
// route). Widget origin is the platform's own domain (this page), which
// stays constant across every page of the client's site, so localStorage
// here persists correctly for a returning visitor on that site.
function getOrCreateVisitorId(siteId: string): string {
  const key = `aiwidget_visitor_${siteId}`;
  try {
    const existing = window.localStorage.getItem(key);
    if (existing) {
      console.log('[Widget] visitorId loaded from localStorage:', existing);
      return existing;
    }
    const fresh =
      'vis_' +
    ...
    console.log('[Widget] visitorId generated and stored:', fresh);
    return fresh;
  } catch (err) {
    // PAIN POINT: localStorage can throw (private browsing, storage
    // disabled, quota exceeded). Fall back to an in-memory id rather than
    // crashing the widget — visitor just won't persist across reloads.
    console.warn('[Widget] localStorage unavailable, using session-only visitorId:', err);
    return 'vis_' + Date.now() + '_' + Math.random().toString(36).slice(2);
  }
}

function getStoredConversationId(siteId: string): string | null {
  try {
    return window.localStorage.getItem(`aiwidget_conversation_${siteId}`);
  } catch {
    return null;
  }
}

function storeConversationId(siteId: string, conversationId: string) {
  try {
    window.localStorage.setItem(`aiwidget_conversation_${siteId}`, conversationId);
  } catch (err) {
    console.warn('[Widget] failed to persist conversationId:', err);
  }
}
...
function WidgetFrameContent() {
  ...
  // Both ids are seeded from localStorage on mount (see useEffect below) so
  // a page refresh mid-conversation still resolves to the same visitor and
  // the same conversation thread instead of starting/joining a new one.
  const visitorIdRef = useRef<string>('');
  const conversationIdRef = useRef<string | null>(null);
  // Mirrors `isTyping` state so the message-listener effect below (which
  // only resubscribes when `searchParams` changes, i.e. effectively once)
  // always reads the current value instead of a stale closure from mount.
  ...
  // before we fall back to generic defaults — a 404 means the site truly
  // wasn't found and we shouldn't waste time retrying that.
  useEffect(() => {
    if (!siteId) return;
    ...
    function fetchConfig(isRetry: boolean) {
      fetch(`/api/v1/widget/config?siteId=${encodeURIComponent(siteId)}`)
        .then(async (res) => {
          if (res.status === 503 && !isRetry) {
            console.warn('[Widget Frame] Config temporarily unavailable, retrying once...');
          ...
          if (res.ok && data && !data.error) {
            ...
            if (!res.ok) console.warn(`[Widget Frame] Config fetch returned ${res.status}`);
            applyFallback();
          }
        })
        .catch((err) => {
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    let aiText = '';
    let aiSources: { title: string; url?: string }[] = [];
    ...
    try {
      ...
      if (!response.ok || !response.body) {
        console.error('[Widget] chat/stream returned non-ok response:', response.status);
      ...
      if (aiSources.length > 0) {
        setMessages((prev) =>
          prev.map((msg) => (msg.id === agentMsgId ? { ...msg, sources: aiSources } : msg))
        );
      }
      ...
      setMessages((prev) =>
        prev.map((msg) => (msg.id === agentMsgId ? { ...msg, text: fallbackText } : msg))
      );
    } finally {
      setIsTyping(false);
    }
    ...
    <>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .input-wrapper input:disabled { cursor: not-allowed; opacity: 0.7; }
        .send-btn {
          width: 34px;
          height: 34px;
          border-radius: 50%;
      ...
      <div className="frame-container">
        ...
        <div className="chat-card">
          ...
          <div className="chat-messages">
            {configLoading ? (
              // PAIN POINT (fixed): the widget used to render a fully blank
              // white panel between iframe mount and the config fetch
              // resolving — no spinner, no skeleton, nothing. On a slow
              // connection this looked broken for up to a couple seconds.
                          ...
                          {msg.sources.map((s, i) => (
                            s.url ? (
                              <a
                                ...
                                className="msg-source-pill"
                              >
                                {s.title}
                              </a>
                            ) : (
                              <span key={i} className="msg-source-pill">{s.title}</span>
                            )
                          ))}
                        </div>
                      )}
        ...
        <div className="input-dock">
          {/* PAIN POINT (fixed): chips used to render unconditionally for
              the entire conversation, permanently cluttering the input
              dock after the visitor had already started chatting. Only
              show them before the visitor's first message. */}
          {messages.filter((m) => m.sender === 'user').length === 0 && (
            <div className="chips-scroll">
              {suggestions.map((chipText, idx) => (
                <button key={idx} className="chip" onClick={() => handleSend(chipText)} disabled={isTyping}>
                  {chipText}
                </button>
              ))}
            </div>
          ...
          <div className="input-wrapper">
            ...
            />
            <button
              className={`send-btn ${inputQuery.trim() && !isTyping ? 'active' : ''}`}
              onClick={() => handleSend()}
