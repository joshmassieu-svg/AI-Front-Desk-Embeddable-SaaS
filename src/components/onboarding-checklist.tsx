...
export function OnboardingChecklist({
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('flowdexx_checklist_dismissed');
  ...
  const handleDismiss = () => {
    setDismissed(true);
    if (typeof window !== 'undefined') {
      localStorage.setItem('flowdexx_checklist_dismissed', 'true');
    }
  };

  if (dismissed) return null;

  // Determine completed steps based on real signals
  ...
  const hasEmbedded =
    ...
    localStorage.getItem('flowdexx_embed_copied') === 'true';

  const isStep1Done = hasKnowledge;
  const isStep2Done = hasCustomized;
  const isStep3Done = hasEmbedded;
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 relative z-10">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-200/70 text-amber-900 text-xs font-bold border border-amber-300/80">
