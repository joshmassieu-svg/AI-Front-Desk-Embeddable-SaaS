...
class DatabaseStore {
  ...
  public async addKnowledgeChunksAsync(chunks: Omit<KnowledgeChunk, 'id' | 'createdAt'>[]): Promise<number> {
    ...
    for (const chunk of chunks) {
      ...
      const record: KnowledgeChunk = { ...chunk, id, createdAt: new Date().toISOString() };
      try {
        await setDoc(doc(firestore, 'knowledgeChunks', id), record);
        saved++;
      } catch (err) {
        console.error('Error saving knowledge chunk to Firestore:', err);
      }
    }
    return saved;
  }
  ...
  public async queryKnowledgeChunksByVectorAsync(
    ...
    if (!firebaseDb) {
      console.error('Vector search failed: Firebase Admin SDK is not initialized (missing service account credentials).');
      return [];
    }
    try {
      ...
      console.error('Vector search failed (check vector index is deployed):', err);
      return [];
    }
  }
