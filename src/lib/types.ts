export interface WebsiteConfig {
  id: string;
  // Owning account. Always set at creation time now — required for the
  // `getUserWebsitesFromFirestore` query and for ownership checks.
  workplaceId?: string;
  userId?: string;
  name: string;
  domain: string;
  ...
  // AI Config
  model: 'gemini-1.5-flash' | 'gemini-1.5-pro' | 'gemini-2.0-flash' | 'gemini-2.5-pro';
  systemPrompt: string;
  temperature: number;
  maxTokens: number;
...
export interface KnowledgeItem {
  ...
  lastSyncedAt: string;
  // Groups pages that came from the same "crawl entire site" job so they
  // can be managed/deleted together in the dashboard.
  crawlJobId?: string;
}
